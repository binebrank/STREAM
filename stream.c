...
main(int argc, char **argv)
    {
    if (argc != 3) {
        printf("./stream_c.exe <input_size> <iterations>\n");
        return 1;
    }
    ...
    n_times = atol(argv[2]);
    if (n_times > NTIMES) {
        printf("Number of iterations too big. Keep it less than %d.\n", NTIMES);
        return 1;
    }
    ...
    a = (double *) malloc(sizeof(STREAM_TYPE) * array_size);
    b = (double *) malloc(sizeof(STREAM_TYPE) * array_size);
    c = (double *) malloc(sizeof(STREAM_TYPE) * array_size);
...
#pragma omp parallel for
    ...
    /*	--- MAIN LOOP --- repeat test cases n_times times --- */
...
#pragma omp parallel for
    ...
    for (k=1; k<n_times; k++) /* note -- skip first iteration */
	{
	for (j=0; j<4; j++)
	    {
